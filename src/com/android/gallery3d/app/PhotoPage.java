...
public abstract class PhotoPage extends ActivityState implements
			...
			if (itemPath == null) {
				...
				if (mediaItemCount > 0) {
					if (mCurrentIndex >= mediaItemCount)
						mCurrentIndex = 0;
					itemPath = mMediaSet.getMediaItem(mCurrentIndex, 1).get(0)
							.getPath();
				} else {
	...
	public boolean canDisplayBottomControl(int control) {
		...
		case R.id.photopage_bottom_control_tiny_planet:
			return mHaveImageEditor && mShowBars && mIsPanorama360
					&& !mPhotoView.getFilmMode();
		default:
			return false;
		}
	...
	private void onUpPressed() {
		...
		if (mAppBridge == null) {
			...
			mActivity.getStateManager()
					.switchState(this, AlbumPage.class, data);
		} else {
	...
	private String getPathFromURI(Uri contentUri) {
		...
		CursorLoader loader = new CursorLoader(mActivity.getAndroidContext(),
	...
	private void setCurrentPhotoByIntent(Intent intent) {
		...
		if (path != null) {
			...
			if (!albumPath.equalsIgnoreCase(mOriginalSetPathString)) {
				...
				mActivity.getStateManager().startState(SinglePhotoPage.class,
						data);
				return;
			}
	...
	public boolean onShareTargetSelected(ShareActionProvider source,
		...
		UsageStatistics.onEvent(UsageStatistics.COMPONENT_GALLERY,
				UsageStatistics.ACTION_SHARE, mediaType,
				timestampMillis > 0 ? System.currentTimeMillis()
						- timestampMillis : -1);
		return false;
	}
